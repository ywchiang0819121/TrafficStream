...
def load_best_model(args):
    if (args.load_first_year and args.year <= args.begin_year+1) or args.train == 0:
        ...
        loss = []
        for filename in os.listdir(osp.join(args.model_path, args.logname, str(args.year-1))):
            loss.append(filename[0:-4])
        loss = sorted(loss)
        # load_path = osp.join(args.model_path, args.logname, str(args.year), loss[0]+".pkl")
        load_path = osp.join(args.model_path, args.logname, str(args.year-1), loss[0]+".pkl")
...
def init(args):
    ...
    vars(args)["path"] = osp.join(args.model_path, args.logname)
    ct.mkdirs(args.path)
    del info
...
def train(inputs, args):
    ...
    if args.loss == "mse":
        ...
        lossfunc = func.poisson_nll_loss
    elif args.loss == "gaussian":
        lossfunc = func.gaussian_nll_loss
    ...
    if args.strategy == 'incremental' and args.year > args.begin_year:
        ...
        train_loader = DataLoader(TrafficDataset(
            inputs, "train"), batch_size=args.batch_size, shuffle=True, pin_memory=pin_memory, num_workers=n_work)
        val_loader = DataLoader(TrafficDataset(inputs, "val"), batch_size=args.batch_size,
                                shuffle=False, pin_memory=pin_memory, num_workers=n_work)
        vars(args)["sub_adj"] = vars(args)["adj"]
    test_loader = DataLoader(TrafficDataset(inputs, "test"), batch_size=args.batch_size,
                             shuffle=False, pin_memory=pin_memory, num_workers=n_work)
    ...
    if args.init == True and args.year > args.begin_year:
        ...
        if args.ewc:
            ...
            model = EWC(gnn_model, args.adj,
                        args.ewc_lambda, args.ewc_strategy)
            ewc_loader = DataLoader(TrafficDataset(
                inputs, "train"), batch_size=args.batch_size, shuffle=False, pin_memory=pin_memory, num_workers=n_work)
        ...
        with torch.no_grad():
            for batch_idx, data in enumerate(val_loader):
                ...
                if args.strategy == "incremental" and args.year > args.begin_year:
                    ...
                    data.y = data.y[:, args.mapping, :]
                loss = masked_mae_np(
                    data.y.cpu().data.numpy(), pred.cpu().data.numpy(), 0)
    ...
    best_model.load_state_dict(torch.load(
        best_model_path, args.device)["model_state_dict"])
    best_model = best_model.to(args.device)

    # Test Model
    test_model(best_model, args, test_loader, pin_memory)
    result[args.year] = {"total_time": total_time, "average_time": sum(
        use_time)/len(use_time), "epoch_num": epoch+1}
    args.logger.info("Finished optimization, total time:{:.2f} s, best model:{}".format(
...
def metric(ground_truth, prediction, args):
    ...
    for i in pred_time:
        ...
        rmse = masked_mse_np(
            ground_truth[:, :, :i], prediction[:, :, :i], 0) ** 0.5
        mape = masked_mape_np(ground_truth[:, :, :i], prediction[:, :, :i], 0)
        args.logger.info(
            "T:{:d}\tMAE\t{:.4f}\tRMSE\t{:.4f}\tMAPE\t{:.4f}".format(i, mae, rmse, mape))
...
def main(args):
    ...
    for year in range(args.begin_year, args.end_year+1):
        ...
        graph = nx.from_numpy_matrix(
            np.load(osp.join(args.graph_path, str(year)+"_adj.npz"))["x"])
        vars(args)["graph_size"] = graph.number_of_nodes()
        vars(args)["year"] = year
        inputs = generate_samples(31, osp.join(args.save_data_path, str(year)+'_30day'), \
            np.load(osp.join(args.raw_data_path, str(year)+".npz"))["x"], graph, val_test_mix=True) \
            if args.data_process else np.load(osp.join(args.save_data_path, str(year)+"_30day.npz"), allow_pickle=True)

        args.logger.info("[*] Year {} load from {}_30day.npz".format(
            args.year, osp.join(args.save_data_path, str(year))))
        ...
        if year == args.begin_year and args.load_first_year:
            ...
            test_loader = DataLoader(TrafficDataset(
                inputs, "test"), batch_size=args.batch_size, shuffle=False, pin_memory=pin_memory, num_workers=n_work)
        ...
        if year > args.begin_year and args.strategy == "incremental":
            ...
            if args.detect:
                args.logger.info(
                    "[*] detect strategy {}".format(args.detect_strategy))
                pre_data = np.load(
                    osp.join(args.raw_data_path, str(year-1)+".npz"))["x"]
                cur_data = np.load(
                    osp.join(args.raw_data_path, str(year)+".npz"))["x"]
            ...
            edge_list = list(nx.from_numpy_matrix(
                np.load(osp.join(args.graph_path, str(year)+"_adj.npz"))["x"]).edges)
        ...
        if args.strategy != "retrain" and year > args.begin_year and len(args.node_list) == 0:
            ...
            ct.mkdirs(osp.join(args.model_path,
                      args.logname, str(args.year)))
            torch.save({'model_state_dict': model.state_dict()}, osp.join(
                args.model_path, args.logname, str(args.year), loss+".pkl"))
            test_loader = DataLoader(TrafficDataset(
                inputs, "test"), batch_size=args.batch_size, shuffle=False, pin_memory=pin_memory, num_workers=n_work)
            test_model(model, args, test_loader, pin_memory=True)
            logger.warning("[*] No increasing nodes at year " +
                           str(args.year) + ", store model of the last year.")
            continue
    ...
    for year in range(args.begin_year, args.end_year+1):
        if year in result:
            info = "year\t{}\ttotal_time\t{}\taverage_time\t{}\tepoch\t{}".format(
                year, result[year]["total_time"], result[year]["average_time"], result[year]['epoch_num'])
            logger.info(info)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--first_year_model_path", type=str,
                        default="res/expansible/2021/306.0995.pkl", help='specify a pretrained model root')
